...
namespace CoverCMS\Socialite\Coding;
...
class OAuth2 extends AbstractOAuth2
{
    ...
    {
        try {
            $response = $this->http->get($this->getUrl('api/oauth/access_token'), array(
                'client_id' => $this->appid,
        ...
        if ($this->isSuccess($this->result)) {
            ...
            throw new Exception($this->getErrorCode($this->result), $this->getErrorMsg($this->result));
        }
    }
    ...
    {
        try {
            $response = $this->http->get($this->getUrl('api/account/current_user', array(
