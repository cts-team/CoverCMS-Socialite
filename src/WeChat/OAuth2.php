...
namespace CoverCMS\Socialite\WeChat;
...
class OAuth2 extends AbstractOAuth2
{
    ...
    {
        try {
            $response = $this->http->get($this->getUrl('sns/oauth2/access_token', array(
                'appid' => $this->appid,
    ...
    {
        try {
            $response = $this->http->get($this->getUrl('sns/userinfo', array(
    ...
    {
        try {
            $response = $this->http->get($this->getUrl('sns/oauth2/refresh_token', array(
                'appid' => $this->appid,
    ...
    {
        try {
            $response = $this->http->get($this->getUrl('sns/auth', array(
                'access_token' => null === $accessToken ? $this->accessToken : $accessToken,
                'openid' => $this->openid,
            )));
        } catch (ClientException $exception) {
    ...
    {
        try {
            $response = $this->http->get($this->getUrl('sns/jscode2session', array(
                'appid' => $this->appid,
                'secret' => $this->appSecret,
                'js_code' => $jsCode,
                'grant_type' => 'authorization_code',
