...
namespace CoverCMS\Socialite\AliPay;


use CoverCMS\Socialite\AbstractOAuth2;
use CoverCMS\Socialite\Exception;
use CoverCMS\Socialite\Library\RSA;
use CoverCMS\Socialite\Library\RSA2;
...
class OAuth2 extends AbstractOAuth2
{
    ...
    protected function __getAccessToken($storeState, $code = null, $state = null)
    {
        ...
        if (!isset($this->result['alipay_system_oauth_token_response']) && isset($this->result['error_response'])) {
            throw new Exception(sprintf('%s %s', $this->result['error_response']['msg'], $this->result['error_response']['sub_msg']), $this->result['error_response']['code']);
        }
        $this->result = $responseData = $this->result['alipay_system_oauth_token_response'];
        if (isset($responseData['code'])) {
            throw new Exception(sprintf('%s %s', $responseData['msg'], $responseData['sub_msg']), $responseData['code']);
        }
